...
/// UpdateStaffMemberRequest : A partial update: an omitted field keeps whatever is stored.  The nullable fields take an explicit `null` to clear the stored value, which is why they are `Option<Option<_>>` — the previous shape wrote a default over every field the caller left out.
...
impl UpdateStaffMemberRequest {
    ...
    pub fn new() -> UpdateStaffMemberRequest {
